...
const numberOfSpeed = [1, 1, 3, 5]
const numberOfRegen = [0, 2, 2, 6]
const numberOfStrength = [0, 1, 1, 4]
...
func toStrengthActivate():
	strengthBossActive = true
	$CanvasLayer/StrengthTimer.start()

func _on_StrengthTimer_timeout():
	strengthBossActive = false
	toStrengthInvisibility()

func toStrengthVisibility():
	$CanvasLayer/ColorRect2/Strength2.visible = true
...
func build_level():
	...
	#Placing the speed potions on the floor randomly
	var numberOfRegenFloor = numberOfRegen[floorNumber] #Sets the amount of speed potions with the respective floor number
	for i in range(numberOfRegenFloor): #Amount of speed potions that it will process
		...
		#Placing the speed potions on the floor randomly
	var numberOfStrengthFloor = numberOfStrength[floorNumber] #Sets the amount of speed potions with the respective floor number
	for i in range(numberOfStrengthFloor): #Amount of speed potions that it will process
...
class Regeneration:
	...
	func _init(game, x, y):
		tile = Vector2(x, y) #Setting the tile location of the speed potion
		regenNode = RegenScene.instance() #Creating instance of the speed potion scene
...
class Strength:
	...
	func _init(game, x, y):
		tile = Vector2(x, y) #Setting the tile location of the speed potion
		strengthNode = StrengthScene.instance() #Creating instance of the speed potion scene
...
func toPlayerRegenPotion():
	$CanvasLayer/RegenTimer.start()
	$CanvasLayer/RegenStopTimer.start()
...
func onBossDamage():
	if strengthBossActive == false:
		...
		BossHealth.bossHealth -= 22#When the player does damage to the boss, the boss health is reduced by 10
		bossUpdateHealth() #Updates the health of the boss so can visually see change
	...
	if BossHealth.bossHealth <= 0:
		...
		if bossDefeated == true and coinsCollected == true and enemiesDefeated == true:
			$Player.queue_free()
			$CanvasLayer/ColorRect.visible = false
			$CanvasLayer/ColorRect2.visible = false
			$CanvasLayer/ColorRect3.visible = false
...
func _on_Restart_pressed():
	...
	$CanvasLayer/ColorRect2/Speed2.visible = false #If speed symbol active when dead, sets it to black
	$CanvasLayer/ColorRect2/Heal2.visible = false
	$CanvasLayer/ColorRect2/Strength2.visible = false
	$CanvasLayer/Lose.visible = false #So you can not see the losers screen anymore
	$CanvasLayer/BossDefeat.visible = false
	$CanvasLayer/WinGame.visible = false
